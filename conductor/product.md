...
2. **Real-Time Gameplay:** Live syncing of votes, online status, and topics using Convex.
3. **The Mask:** Individual vote values remain hidden until the Facilitator triggers the reveal.
4. **Results & Statistics:** Automatic calculation of average, median, mode, and consensus level, highlighting outliers for discussion.
5. **Interactive Feedback:** Fun elements like confetti on unanimous votes and real-time emoji reactions.
6. **Facilitator Controls:** Full control over topic queues, resetting rounds, and revealing votes.
...
A "Modern Corporate" aesthetic featuring deep shadows, subtle glassmorphism, and smooth 3D animations (like card flips). The experience should feel responsive and app-like, even serving as a standalone PWA on mobile devices.